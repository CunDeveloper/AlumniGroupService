...
import com.nju.edu.cn.software.mapper.RecommendCollectMapper;
import com.nju.edu.cn.software.util.CustomExceptionUtil;
import com.nju.edu.cn.software.util.ExceptionLogUtil;

@Path(PathConstant.RECOMMEND_COLLECT)
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)

public class RecommendCollectService {

	private static final Logger log = Logger.getLogger(RecommendCollectService.class);
	 
	@Autowired
	private RecommendCollectMapper collectDao;
	
	@POST
	@Path(PathConstant.SAVE_RECOMMEND_COLLECT)
	public Response  saveRecommendCollect(ReqCollectIdEntity reqEntity,@Context ContainerRequestContext crc){
		try{
			int authorId = (int) crc.getProperty(Constant.AUTHOR_ID);
			RecommendCollect collect = new RecommendCollect();
			collect.setAuthorId(authorId);
			RecommendWork work = new RecommendWork();
			work.setId(reqEntity.getCollectId());
			collect.setRecommendWork(work);
	...
	@POST
	@Path(PathConstant.DELETE_RECOMMEND_COLLECT)
	public Response deleteRecommendCollect(ReqIdEntity reqEntity,@Context ContainerRequestContext crc){
		try{
			int authorId = (int) crc.getProperty(Constant.AUTHOR_ID);
	...
	@POST
	@Path(PathConstant.GET_RECOMMEND_COLLECT)
	public Response  getRecommendCollects(@Context ContainerRequestContext crc){
		try{
			int authorId = (int) crc.getProperty(Constant.AUTHOR_ID);
			List<RecommendCollect> collects = collectDao.getCollects(authorId);
